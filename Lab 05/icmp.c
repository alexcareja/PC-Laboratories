...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	memcpy(pkt.payload + sizeof(icmp_hdr) + ICMP_OFF, data, sizeof(data));
	...
	ip_hdr->tos = 0;
	ip_hdr->tot_len = htons(sizeof(struct icmphdr) + sizeof(struct iphdr) + sizeof(data));
	ip_hdr->id = htons(i);
	ip_hdr->frag_off = 0;
	ip_hdr->ttl = 255;
	ip_hdr->protocol = 1;
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	struct icmphdr *ricmp_hdr = (struct icmphdr *)(reply.payload + ICMP_OFF);
	struct in_addr x;
	x.s_addr = rip_hdr->saddr;
	ip_s = inet_ntoa(x);
	printf("%d %s\n", i, ip_s);
	if (ricmp_hdr->type == 0) {	// asta e reply
		break;
	}
